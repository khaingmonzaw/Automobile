...
`;
    ...
    db.query(checkUserSql, [email, driverLicense], async (err, userResult) => {
      if (err) {
      ...
      if (userResult.length > 0) {
        ...
        if (
          existingUser.driver_license === driverLicense &&
...
`,
              ...
              (err, policyResult) => {
                ...
                if (policyResult.length > 0) {
                  return res.status(400).json({
                    message: "This vehicle already has an active policy.",
                  });
                }
...
`,
              ...
              (err, result) => {
                ...
                savePolicy(result.insertId, userId);
              },
            );
          }
        },
